...
void filter_detections_by_classes(detection *dets, int num, int *target_classes, int num_target_classes) {
    for (int i = 0; i < num; ++i) {
        int keep = 0;
        for (int j = 0; j < num_target_classes; ++j) {
            if (dets[i].prob[target_classes[j]] > 0) {
                keep = 1;
                break;
            }
        }
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh,
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers, int *target_classes, int num_target_classes)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    int names_size = 0;
    ...
    while (1) {
        ...
        // 필터링 함수 호출
        // filter_detections_by_classes(dets, nboxes, target_classes, num_target_classes);
        draw_detections_v3(im, dets, nboxes, thresh, names, alphabet, l.classes, ext_output, target_classes, num_target_classes);
...
void draw_object(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, int dont_show, int it_num,
    int letter_box, int benchmark_layers, int *target_classes, int num_target_classes)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    int names_size = 0;
    ...
    while (1) {
        ...
        draw_detections_v3(sized, dets, nboxes, thresh, names, alphabet, l.classes, 1, target_classes, num_target_classes);
...
void run_detector(int argc, char **argv)
{
    ...
    // 타겟 클래스 인수 추가
    int target_classes[80];
    int num_target_classes = 0;
    char *target_classes_str = find_char_arg(argc, argv, "-target_classes", 0);
    if (target_classes_str) {
        char *token = strtok(target_classes_str, ",");
        while (token != NULL) {
            target_classes[num_target_classes++] = atoi(token);
            token = strtok(NULL, ",");
        }
    }

    if (0 == strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile, letter_box, benchmark_layers, target_classes, num_target_classes);
        ...
        int it_num = 100;
        draw_object(datacfg, cfg, weights, filename, thresh, dont_show, it_num, letter_box, benchmark_layers, target_classes, num_target_classes);
    }
