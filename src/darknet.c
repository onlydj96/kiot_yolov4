...
int main(int argc, char **argv)
{
    ...
    if (0 == strcmp(argv[1], "average")){
        ...
        // 외부에서 전달받은 클래스 ID를 처리합니다.
        int target_classes[20]; // 최대 20개의 클래스를 받을 수 있도록 배열 크기를 설정합니다.
        int num_target_classes = 0;
        for (int i = 5; i < argc; ++i) { 
            if (strncmp(argv[i], "target_classes=", 15) == 0) {
                char *class_ids = argv[i] + 15; // "target_classes=" 이후의 문자열을 얻습니다.
                char *token = strtok(class_ids, " ");
                while (token != NULL && num_target_classes < 20) {
        ...
        test_detector("cfg/coco.data", argv[2], argv[3], filename, thresh, 0.5, 0, ext_output, 0, NULL, 0, 0, target_classes, num_target_classes);
